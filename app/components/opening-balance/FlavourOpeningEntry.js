...
class FlavourOpeningEntry extends Authentication {
    ...
    redirectToCreate() {
        ...
        browserHistory.push("/createFlavourOpening");
    }

    redirectToEdit(currId) {
      let currProps = this.props;
      browserHistory.push({
        pathname: "/createFlavourOpening",
        state: {
          upsertAction: "update",
          id: currId
        }
    ...
    render() {
      ...
      const columns = [{
        ...
        Header: 'Flavour',
        accessor: 'flavour', // String-based value accessors!
        filterMethod: (filter, rows) =>
          matchSorter(rows, filter.value, { keys: ["flavour"] }),
        filterAll: true
      }, {
